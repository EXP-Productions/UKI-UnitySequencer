...
namespace Unity.BlinkyBlinky
{
    public static class DatagramComposer
    {
        ...
        public static DMXDatagram[] UpdateDMXDatagramBuffers(Fixture fixture, DMXDatagram[] datagrams)
        {
            int bufferCount = 0;
            for (int i = 0; i < fixture.LedChains.Count; i++)
            {
                LedChain chain = fixture.LedChains[i];
                ...
                while (universesProcessed < chain.DmxUniversesRequired)
                {
                    ...
                    while (pixelIndex < 170)
                    {
                        ...
                        datagrams[bufferCount].Buffer[idx] = (byte)chain.Pixels[universePixelIndex].R;
                        datagrams[bufferCount].Buffer[idx + 1] = (byte)chain.Pixels[universePixelIndex].G;
                        datagrams[bufferCount].Buffer[idx + 2] = (byte)chain.Pixels[universePixelIndex].B;
                ...
                while (pixelIndexLast < remainingPixelsToProcess)
                {
                    ...
                    datagrams[bufferCount].Buffer[idx] = (byte)chain.Pixels[pixelIndexFinal].R;
                    datagrams[bufferCount].Buffer[idx + 1] = (byte)chain.Pixels[pixelIndexFinal].G;
                    datagrams[bufferCount].Buffer[idx + 2] = (byte)chain.Pixels[pixelIndexFinal].B;
        ...
        public static DMXDatagram[] InitDMXDatagrams(Fixture fixture)
        {
